...
# 5. Highlight "Curvature Condition" region
y_curvature = copy(phi.(alphas))
mask_curvature = dphi.(alphas) .>= sigma * slope0
y_curvature[.!mask_curvature] .= NaN


plot!(p1, alphas, y_curvature, 
    label="Curvature condition satisfied", color=color_highlight, linewidth=4.5
)
...
scatter!(p1, [0.0], [phi(0.0)], label=nothing, color=:black, markersize=5)
...
display(p1)
savefig("curvature_condition.pdf")